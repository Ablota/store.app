	...
	<f7-page name="source">
		<f7-navbar :back-link="$t('words.back')" large transparent>
			...
			<f7-nav-right v-if="source">
				<f7-link v-if="qrcode" icon-aurora="f7:qrcode" icon-ios="f7:qrcode" icon-md="f7:qrcode" popover-open=".popover-qrcode"></f7-link>
				<f7-link icon-aurora="f7:square_arrow_up" icon-ios="f7:square_arrow_up" icon-md="material:share" @click="submitShare"></f7-link>
				<f7-link v-if="!source.system" icon-aurora="f7:trash" icon-ios="f7:trash" icon-md="material:delete" @click="submitRemove"></f7-link>
			</f7-nav-right>
		</f7-navbar>
		...
		<div v-if="source">
			<f7-list accordion-list class="no-margin" media-list no-hairlines>
				...
				})" :subtitle="source.repo.description" accordion-item class="no-subtitle-limit"
				>
					<img v-if="icon" slot="media" :src="icon" alt="" class="icon-60"/>
					<f7-skeleton-block v-if="source.repo.icon && !icon" slot="media" class="icon-60"/>
					<f7-accordion-content>
						...
						<div v-if="source.repo.mirrors.length">
							...
							<f7-list class="no-title-limit" inset>
								<f7-list-item v-for="mirror in source.repo.mirrors" :key="mirror" :title="mirror"></f7-list-item>
							</f7-list>
						</div>
						...
						<f7-list class="no-title-limit" inset>
							<f7-list-item :header="$t('words.sha256')" :title="source.fingerprint.match(/.{1,2}/g).join(':').toUpperCase()"></f7-list-item>
						</f7-list>
					</f7-accordion-content>
				</f7-list-item>
			</f7-list>
		...
		<div v-else>
			<f7-list class="no-margin" media-list no-hairlines>
				<f7-list-item
					:footer="$t('pages.source.footer', {
						...
						version: 1,
					})" class="no-subtitle-limit skeleton-text" subtitle="Lorem ipsum dolor sit amet, consetetur sadipscing elitr."
				>
			...
			<f7-list>
				<f7-list-item
					v-for="index in 5" :key="index" :footer="$tc('counts.apps', 0, {
						count: 0,
					})" class="skeleton-text" title="Lorem ipsum."
				></f7-list-item>
			</f7-list>
		</div>
	</f7-page>
