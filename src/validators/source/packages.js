...
export default Joi.object().pattern(Joi.any(), Joi.array().items(Joi.object({
	...
	'uses-permission': Joi.array().items(Joi.array().items(Joi.string().trim().required(), Joi.number().integer().positive().allow(null).default(null).optional()).default([]).optional()).default([]).optional(),
	'uses-permission-sdk-23': Joi.array().items(Joi.array().items(Joi.string().trim().required(), Joi.number().integer().positive().allow(null).default(null).optional()).default([]).optional()).default([]).optional(),
